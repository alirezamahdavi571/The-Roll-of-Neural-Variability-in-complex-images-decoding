function [accuracy_w, accuracy_w_mat] = lda_classifier_psth(artifact_data, body_data, natural_data, face_data, number_of_time_slices, region)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % SVM Classifier on PSTH in 4 Categories
    % Inputs:
    %   - artifact_data: Artifact PSTH data (samples x features x time slices)
    ...
    %   - recall_artifact: Recall for artifact category across time slices
    ...
    for ts = 1:number_of_time_slices
        ...
        artifact_features = transpose(artifact_data(3:end, :, ts));
        body_features = transpose(body_data(3:end, :, ts));
        natural_features = transpose(natural_data(3:end, :, ts));
        face_features = transpose(face_data(3:end, :, ts));
    ...
    % Save recall data
    save(['Recall_animate_rate_', region, '.mat'], 'recall_artifact', '-v7.3');
